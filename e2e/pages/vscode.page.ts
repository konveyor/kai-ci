...
export class VSCode extends BasePage {
  ...
  public async openLeftBarElement(name: LeftBarItems) {
    ...
    const moreButton = window
      .getByRole('button', { name: LeftBarItems.AdditionalViews })
      .first();
    await expect(moreButton).toBeVisible();
    await moreButton.click();

    const menuBtn = window.locator(
      `a.action-menu-item span[aria-label^="${name}"]`
    );
  ...
  public async updateLLMCache() {
    const newCacheZip = path.join(
      path.dirname(this.llmCachePaths().storedPath),
      'new.zip'
    );
    createZip(this.llmCachePaths().workspacePath, newCacheZip);
    fs.renameSync(newCacheZip, this.llmCachePaths().storedPath);
    fs.renameSync(
      `${newCacheZip}.metadata`,
      `${this.llmCachePaths().storedPath}.metadata`
    );
  }
   ...
   */
  public async writeOrUpdateVSCodeSettings(
