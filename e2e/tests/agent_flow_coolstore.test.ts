...
providers.forEach((config) => {
  test.describe(`Coolstore app tests with agent mode enabled - offline (cached) | ${config.provider}/${config.model}`, () => {
    ...
    test.beforeAll(
      async ({ testRepoData }: { testRepoData: any }, testInfo: any) => {
        ...
        try {
          ...
          console.log(
            `An existing profile probably doesn't exist, creating a new one`
          );
        }
    ...
    test('Fix JMS Topic issue with agent mode enabled (offline)', async () => {
      ...
      await vscodeApp.searchViolation(
        'References to JavaEE/JakartaEE JMS elements'
      );
      ...
      while (!done) {
        ...
        const yesButton = resolutionView
          .locator('button')
          .filter({ hasText: 'Yes' });
