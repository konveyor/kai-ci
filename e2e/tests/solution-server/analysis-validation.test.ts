...
test.describe(`Solution server analysis validations`, () => {
  ...
  test.beforeAll(async ({ testRepoData }) => {
    ...
    await config.setEnabledConfiguration(
      ConfigurationOptions.SolutionServerEnabled,
      true
    );
    ...
    await expect(
      vsCode.getWindow().getByText('Analysis completed').first()
    ).toBeVisible({
      timeout: 300000,
    });
  });
  ...
  test.beforeEach(async () => {
    ...
    bestHintBase = await mcpClient.getBestHint(
      'eap8/eap7',
  ...
  async function requestFixAndAssertSolution(accept: boolean) {
    ...
    expect(successRate.pending_solutions).toBe(
      successRateBase.pending_solutions + 1
    );
    expect(successRate.counted_solutions).toBe(
      successRateBase.counted_solutions + 1
    );
    ...
    expect(successRate.pending_solutions).toBe(
      successRateBase.pending_solutions
    );
